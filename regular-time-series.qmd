...
`fft` returns a vector of complex numbers.

```{r}
delta_t <- 1/12
spec <- abs(fft_sun)^2 * (delta_t/length(sunspot))
freq <- (0:(length(sunspot)-1)) / (delta_t * length(sunspot))
```

This contains both the positive and negative frequencies
...
gg_spec(list(
  SpecACF = SpecACF(sunspot, bin.width = 1/12, detrend = FALSE),
  SpecMTM = SpecMTM(sunspot, detrend = FALSE),
  FFT = spec_sun)
  ) +
